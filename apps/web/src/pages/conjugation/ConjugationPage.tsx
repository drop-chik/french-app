...
const PRONOUNS_RU = ['я', 'ты', 'он/она', 'мы', 'вы', 'они'];
const PRONOUNS_EN = ['I', 'you', 'he/she', 'we', 'you', 'they'];
const IMPERATIVE_PRONOUNS_RU = ['(ты)', '(мы)', '(вы)'];
const IMPERATIVE_PRONOUNS_EN = ['(you)', '(let\'s)', '(you pl.)'];
...
export function ConjugationPage() {
  ...
  const pronouns = lang === 'ru' ? PRONOUNS_RU : PRONOUNS_EN;
  const imperativePronouns = lang === 'ru' ? IMPERATIVE_PRONOUNS_RU : IMPERATIVE_PRONOUNS_EN;
              ...
              <div className={styles.tensesGrid}>
                {TENSE_ORDER.map(({ key, level }) => (
                  <TenseCard
                    ...
                    level={level}
                    pronouns={pronouns}
                    forms={data.tenses[key]}
                  />
                ))}
                <TenseCard
                  title={t.conjugation.tenses.imperatif}
                  level="A1"
                  pronouns={imperativePronouns}
    ...
    <div className={styles.tenseCard}>
      ...
      <table className={styles.table}>
        <tbody>
          {pronouns.map((p, i) => (
            <tr key={p + i}>
              <td className={styles.pronounCell}>{p}</td>
              <td className={styles.formCell}>{forms[i] ?? '—'}</td>
            </tr>
          ))}
        </tbody>
      </table>
