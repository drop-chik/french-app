...
async function seed() {
  ...
  const a1Rows = buildWordRows([...wordsA1, ...wordsA1Extra, ...wordsA1Extra2, ...wordsA1Extra3, ...wordsA1Extra4] as WordInput[], 'A1');
  await seedWordsBatch(a1Rows, 'A1 words');
  console.log(`Words A1 done! Total: ${a1Rows.length}`);

  // ===== Words A2 =====
  console.log('\nSeeding words A2...');
  const a2Rows = buildWordRows([...wordsA2, ...wordsA2Extra, ...wordsA2Extra2, ...wordsA2Extra3, ...wordsA2Extra4, ...wordsA2Extra5] as WordInput[], 'A2');
